...
## Barplots
All barplots will be dram from the same function below, for consistency.

```{r echo=F}
# a helper funtion to draw barplots for categories
barplot_category = function(mcategory) {
  codings %>%
    group_by(category, codename) %>%
    summarise(n = n()) %>%
    filter(category == mcategory) %>%
    arrange(category, -n) %>%
    mutate(codename = fct_inorder(codename)) %>%
    ggplot() +
    aes(x = codename, y = n) + 
...
```{r}
barplot_category("relationshiptype")
barplot_category("opportunity")
barplot_category("motivation")
barplot_category("lock")
barplot_category("process")
barplot_category("knowledge")
barplot_category("aftermath")
barplot_category("status")
```
