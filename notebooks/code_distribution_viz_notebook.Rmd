...
categories_ordered
```
This ordering is useful, for instance, for ordering the columns in the `codings_wide` dataset: 
```{r}
codings_wide = 
  codings_wide %>%
...
Instead of looking just at the mean, we can also see the distribution of relative start characters across categories with a violin plot. 

> TODO: try a crossbar geom, which will require some prior computation of mean and sd (or other in the visualizing error section)
