...
# Data
Analysis is based on the `codings` dataset, which looks like this:
...
### Factor categories
Categories for which rater were only able to select one code translate simply to categorical variables, or factors. Each code, or code-subcode combination represents a level of that factor.
...
categories_ordered
```
The ordered categories are stored as `categories_ordered` to recover the ordering in subsequent visualizations.  This ordering is useful, for instance, for ordering the columns in the `codings_wide` dataset: 
```{r}
codings_wide = 
  codings_wide %>%
...
```
Alternative, with the point range geom and flipped:
...
A more succint visualization of this structural property could be made, but it seems unnecessary for now. (I previously tried `ggraph`, with a circular representation, and was not happy with the results.) The structure does reflect our observations of how the stories usually progressed, such as:
...
The dataset does not distinguish between codes or subcodes: whatever was the lowest possible selectable rating, was the one attributed by raters. First we'll look at the distribution of these codes or subcodes, and then do the same with subcodes consolidated into codes.
...
```
> TODO: revisit single plot with subcodes aggregated

As expected, all categories, except for `process` and `aftermath`, have exactly has many observations. Category `process` has a higher number, because it was common that more than one process was involved in a story. Category `aftermath` has fewer observations, because many stories did not express emotional consequences to the level required by the coding criteria.

Next, we'll look at each category at a time. Barplots will be created with the same function (below) for consistency.
```{r}
barplot_category = function(data_subset) { 
  ggplot(data = data_subset) + 
  geom_bar(
    aes(
      x = fct_infreq(codename),
      fill = code
    )
  ) +
  coord_flip() +
...
> TODO: try faceting again

### relationshiptype
The following types of relationship were found:
```{r}
codings %>%
...
```
Observations:
* In the majority of stories, the relationship between subjects was coded as `intimate`. 
