...
get_barplot_grid = function(data_subset) {
    cowplot::plot_grid(
...
get_barplot_codenames = function(data_subset, width = 50) { 
  data_subset %>% 
  mutate(
    variable = glue(
      "[{codename}] \n {wraped_desc}",
...
- Remove "-NA"" in axis of bar charts for aggregated codes.

- Explore adding the code description as either axes or labels; 

- Re-inspect codings in RQDA.
...
get_barplot_codenames(codings %>% filter(category == "status"))
get_barplot_aggregated(codings %>% filter(category == "status"))
codings %>% filter(category == "process") %>% get_barplot_grid
